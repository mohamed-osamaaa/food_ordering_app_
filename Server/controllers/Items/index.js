...
const deleteSize = async (req, res) => {
    const { itemId, sizeValue } = req.params;

    try {
        const updatedItem = await Item.findByIdAndUpdate(
            itemId,
            {
                $pull: { sizes: { size: sizeValue } },
            },
            { new: true }
        );
...
const getItem = async (req, res) => {
    const { itemId } = req.params;
    try {
        const item = await Item.findById(itemId).populate('category', 'name -_id');
        if (!item) {
            return res.status(404).json({
                success: false,
                message: "Item not found",
...
const getItems = async (req, res) => {
    try {
        const items = await Item.find().populate('category', 'name -_id');
        res.status(200).json({
            success: true,
            data: items,
        });
